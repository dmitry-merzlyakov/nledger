...
function Get-NetFrameworkSDKs {
  ...
  function Get-TfmCodeVersion {
    ...
    [string]$mode = if($isDebug){"d"}else{"r"}
    if ($code -match "^net(\d)(\d+)$") { "$mode-f$($Matches[1].PadLeft(2, '0')).$($Matches[2].PadRight(3, '0'))" }
    if ($code -match "^net(\d+)\.(\d+)$") { "$mode-n$($Matches[1].PadLeft(2, '0')).$($Matches[2].PadRight(3, '0'))" }
    if ($code -match "^netcoreapp(\d+)\.(\d+)$") { "$mode-n$($Matches[1].PadLeft(2, '0')).$($Matches[2].PadRight(3, '0'))" }
  }
